...
class DispersionWidget(pg.PlotWidget):
    ...
    def plotData(self, points, pointsData, xInter, yInter):
        ...
        if self.singleQPoint:
            for i, y in enumerate(np.array(points)[:, 1]):
                line = pg.InfiniteLine(pos = y, angle = 0, movable = False, pen = 'k')
                self.addItem(line)
    ...
    @Slot()
    def getSelectedQPoints(self, item, points, event):
        self.qPointSelected.emit(points, event.modifiers() == Qt.KeyboardModifier.ControlModifier)
