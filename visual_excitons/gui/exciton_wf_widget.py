...
from visual_excitons.core.atomic_data import covalent_radii, jmol_colors
...
class ExcitonWfWidget(gl.GLViewWidget):
    def __init__(self, parent = None):
        ...
        self.isoGlOptions = {
            ...
            GL.GL_BLEND: True,
            # 'glBlendFuncSeparate': (GL.GL_SRC_ALPHA, GL.GL_ONE_MINUS_SRC_ALPHA, GL.GL_ONE, GL.GL_ZERO),
    ...
    def plotWfIso(self, wf, level, transform):
        ...
        self.wfMeshItem.setTransform(tr)
    ...
    def plotWfIsoSet(self, wf, numLevels, transform):
        ...
        for level in np.linspace(0.01, 0.99, numLevels):
            ...
            self.wfIsoVerticesSet.append(verts)
            self.wfIsoFacesSet.append(faces)

            color = self.colormap.mapToQColor(level)
            color.setAlphaF(0.8)
            # colors = np.full(shape=(len(verts), 4), fill_value=[color.redF(), color.greenF(), color.blueF(), 0.5], dtype=np.float32)

            meshdata = gl.MeshData(vertexes=verts, faces=faces)

            meshItem = gl.GLMeshItem(meshdata=meshdata, smooth=True, shader='balloon', glOptions=self.isoGlOptions, drawEdges=False)
            meshItem.setColor((color.redF(), color.greenF(), color.blueF(), color.alphaF()))
            meshItem.setTransform(tr)
    ...
    def plotWfVol(self, wf, sliceDensity, transform):
        ...
        tr = pg.Transform3D(transform)
        self.wfVolItem.setTransform(tr)
    ...
    def plotBaseLatticeVectors(self, baseVectors):
        ...
        self.baseLatticeItems.append(gl.GLLinePlotItem(pos=[[0.0, 0.0, 0.0], baseVectors[0]], color=(1.0, 0.0, 0.0, 1.0), width=1.0, antialias=True, mode='lines'))
        self.baseLatticeItems.append(gl.GLLinePlotItem(pos=[[0.0, 0.0, 0.0], baseVectors[1]], color=(0.0, 1.0, 0.0, 1.0), width=1.0, antialias=True, mode='lines'))
        self.baseLatticeItems.append(gl.GLLinePlotItem(pos=[[0.0, 0.0, 0.0], baseVectors[2]], color=(0.0, 0.0, 1.0, 1.0), width=1.0, antialias=True, mode='lines'))
    ...
    def plotUnitCellBoundaries(self, baseVectors, offset):
        ...
        ucellBoundaries = []
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[[0.0, 0.0, 0.0], baseVectors[0]], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[[0.0, 0.0, 0.0], baseVectors[1]], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[[0.0, 0.0, 0.0], baseVectors[2]], color=color, width=1.0, antialias=True, mode='lines'))

        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[0], baseVectors[0] + baseVectors[1]], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[0], baseVectors[0] + baseVectors[2]], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[1], baseVectors[1] + baseVectors[0]], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[1], baseVectors[1] + baseVectors[2]], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[2], baseVectors[2] + baseVectors[0]], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[2], baseVectors[2] + baseVectors[1]], color=color, width=1.0, antialias=True, mode='lines'))

        apex = baseVectors[0] + baseVectors[1] + baseVectors[2]

        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[0] + baseVectors[1], apex], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[0] + baseVectors[2], apex], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[1] + baseVectors[0], apex], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[1] + baseVectors[2], apex], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[2] + baseVectors[0], apex], color=color, width=1.0, antialias=True, mode='lines'))
        ucellBoundaries.append(gl.GLLinePlotItem(pos=[baseVectors[2] + baseVectors[1], apex], color=color, width=1.0, antialias=True, mode='lines'))
    ...
    def plotUnitCellAtoms(self, atomicNumbers, atomicPositions, offset):
        ...
        for i in range(numAtoms):
            ...
            meshItem.translate(offset[0] + atomicPositions[i][0], offset[1] + atomicPositions[i][1], offset[2] + atomicPositions[i][2])
    ...
    def onCameraChanged(self):
        ...
        if self.wfMeshItem is not None and self.wfMeshItem.visible():
            self.wfMeshItem.setMeshData(meshdata=self.sortedMeshData(self.wfIsoVertices, self.wfIsoFaces, cam), smooth=True)
    ...
    def setOpacity(self, opacity: float):
        if len(self.wfMeshItemSet) > 0:
            for i in range(len(self.wfMeshItemSet)):
                if self.wfMeshItemSet[i].visible():
                    self.wfIsoColorsSet[i].setAlphaF(opacity)
                    color = self.wfIsoColorsSet[i]
                    self.wfMeshItemSet[i].setColor((color.redF(), color.greenF(), color.blueF(), color.alphaF()))
            self.update()